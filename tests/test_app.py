...
def test_put_create_task(client):
    response = client.post(
        "/tasks", data=json.dumps(data_3), content_type="application/json"
    )
    task_id = response.get_json()["id"]
    data_put = {"title": "Test Put", "description": "This is a test task"}
    client.put(
        f"/tasks/{task_id}", data=json.dumps(data_put), content_type="application/json"
    )
...
def test_delete_create_task(client):
    response = client.post(
        "/tasks", data=json.dumps(data_2), content_type="application/json"
    )
    task_id = response.get_json()["id"]
